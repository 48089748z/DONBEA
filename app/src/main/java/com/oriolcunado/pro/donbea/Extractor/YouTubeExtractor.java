...
public abstract class YouTubeExtractor extends AsyncTask<String, Void, SparseArray<YtFile>> {
    ...
    @Override
    protected void onPostExecute(SparseArray<YtFile> ytFiles) {onExtractionComplete(ytFiles, videoMeta);}
    ...
    protected SparseArray<YtFile> doInBackground(String... params)
    {
        ...
        if (mat.find()) {videoID = mat.group(3);}
        else
            {
            mat = patYouTubeShortLink.matcher(ytUrl);
            if (mat.find()) {videoID = mat.group(3);}
            else if (ytUrl.matches("\\p{Graph}+?")) {videoID = ytUrl;}
        }
        if (videoID != null)
        {
            try {return getStreamUrls();}
            catch (Exception e) {e.printStackTrace();}
        }
        else {Log.e(LOG_TAG, "Wrong YouTube link format");}
        return null;
    }
    ...
    private SparseArray<YtFile> getStreamUrls() throws IOException, InterruptedException
    {
        ...
        if (streamMap == null || !streamMap.contains("use_cipher_signature=False"))
        {
            ...
            if (CACHING && (decipherJsFileName == null || decipherFunctions == null || decipherFunctionName == null)) {readDecipherFunctFromCache();}
            ...
            if (parseDashManifest)
            {
                ...
                if (mat.find())
                {
                    ...
                    if (mat.find()) {encSignatures.append(0, mat.group(1));}
                    else {dashMpdUrl = null;}
                }
            }
        }
        else
        {
            if (parseDashManifest)
            {
                ...
                if (mat.find()) {dashMpdUrl = URLDecoder.decode(mat.group(1), "UTF-8");}
            }
        ...
        {
            encStream = encStream + ",";
            if (!encStream.contains("itag%3D")) {continue;}
            String stream;
            ...
            if (mat.find())
            {
                ...
                else if (!includeWebM && FORMAT_MAP.get(itag).getExt().equals("webm")) {continue;}
            }
            else {continue;}
            ...
            if (curJsFileName != null)
            {
                ...
                if (mat.find()) {encSignatures.append(itag, mat.group(1));}
            }
            mat = patUrl.matcher(encStream);
            String url = null;
            if (mat.find()) {url = mat.group(1);}
            if (url != null)
            {
        ...
        {
            if (LOGGING) Log.d(LOG_TAG, "Decipher signatures");
            String signature;
            decipheredSignature = null;
            if (decipherSignature(encSignatures))
            {
                lock.lock();
                try {jsExecuting.await(7, TimeUnit.SECONDS);}
                finally {lock.unlock();}
            }
            ...
            else
            {
                ...
                for (int i = 0; i < encSignatures.size() && i < sigs.length; i++)
                {
                    ...
                    if (key == 0) {dashMpdUrl = dashMpdUrl.replace("/s/" + encSignatures.get(key), "/signature/" + sigs[i]);}
                    else
                    {
        ...
        if (parseDashManifest && dashMpdUrl != null)
        {
            for (int i = 0; i < DASH_PARSE_RETRIES; i++)
            {
                ...
                {
                    Thread.sleep(5);
                    if (LOGGING) Log.d(LOG_TAG, "Failed to parse dash manifest " + (i + 1));
                }
            }
        }
        if (ytFiles.size() == 0)
    ...
    private boolean decipherSignature(final SparseArray<String> encSignatures) throws IOException
    {
        ...
        if (decipherFunctionName == null || decipherFunctions == null)
        {
            ...
            if (LOGGING) Log.d(LOG_TAG, "Decipher FunctURL: " + decipherFunctUrl);
            ...
            if (mat.find())
            {
                ...
                if (mat.find()) {mainDecipherFunct = "var " + decipherFunctionName + mat.group(2);}
                else
                {
                ...
                for (int braces = 1, i = startIndex; i < javascriptFile.length(); i++)
                {
                    ...
                    if (javascriptFile.charAt(i) == '{') braces++;
                    else if (javascriptFile.charAt(i) == '}') braces--;
                }
                ...
                if (CACHING) {writeDeciperFunctToChache();}
            }
            else {return false;}
        }
        else {decipherViaWebView(encSignatures);}
        return true;
    }
    ...
    private void parseDashManifest(String dashMpdUrl, SparseArray<YtFile> ytFiles) throws IOException
    {
        ...
        while (mat.find())
        {
            ...
            if (mat2.find())
            {
                ...
                if (FORMAT_MAP.get(itag) == null) continue;
                if (!includeWebM && FORMAT_MAP.get(itag).getExt().equals("webm")) continue;
            }
            else {continue;}
    ...
    private void parseVideoMeta(String getVideoInfo) throws UnsupportedEncodingException
    {
        ...
        if (mat.find()) {title = URLDecoder.decode(mat.group(1), "UTF-8");}
        mat = patHlsvp.matcher(getVideoInfo);
        if(mat.find()) isLiveStream = true;
        mat = patAuthor.matcher(getVideoInfo);
        if (mat.find()) {author = URLDecoder.decode(mat.group(1), "UTF-8");}
        mat = patChannelId.matcher(getVideoInfo);
        if (mat.find()) {channelId = mat.group(1);}
        mat = patLength.matcher(getVideoInfo);
        if (mat.find()) {length = Long.parseLong(mat.group(1));}
        mat = patViewCount.matcher(getVideoInfo);
        if (mat.find()) {viewCount = Long.parseLong(mat.group(1));}
     ...
     */
    public void setParseDashManifest(boolean parseDashManifest) {this.parseDashManifest = parseDashManifest;}
    /**
     ...
     * this.
     */
    public void setDefaultHttpProtocol(boolean useHttp) {this.useHttp = useHttp;}
